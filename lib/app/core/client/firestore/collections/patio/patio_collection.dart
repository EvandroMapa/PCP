import 'package:aco_plus/app/core/client/firestore/collections/patio/models/patio_model.dart';
import 'package:aco_plus/app/core/models/app_stream.dart';
import 'package:cloud_firestore/cloud_firestore.dart';

class PatioCollection {
  static final PatioCollection _instance = PatioCollection._();

  PatioCollection._();
  PatioCollection.base(); // Constructor for inheritance

  factory PatioCollection() => _instance;
  String name = 'patios';
  String get tableName => name;

  AppStream<List<PatioModel>> dataStream = AppStream<List<PatioModel>>.seed([]);
  List<PatioModel> get data => dataStream.value;

  CollectionReference<Map<String, dynamic>> get collection =>
      FirebaseFirestore.instance.collection(name);

  Future<void> fetch({bool lock = true, GetOptions? options}) async {
    _isStarted = false;
    await start(lock: false, options: options);
    _isStarted = true;
  }

  bool _isStarted = false;
  Future<void> start({bool lock = true, GetOptions? options}) async {
    if (_isStarted && lock) return;
    _isStarted = true;
    final data = await FirebaseFirestore.instance.collection(name).get();
    final patios =
        data.docs.map((e) => PatioModel.fromMap(e.data())).toList();
    patios.sort((a, b) => a.createdAt.compareTo(b.createdAt));
    dataStream.add(patios);
  }

  bool _isListen = false;
  Future<void> listen({
    Object? field,
    Object? isEqualTo,
    Object? isNotEqualTo,
    Object? isLessThan,
    Object? isLessThanOrEqualTo,
    Object? isGreaterThan,
    Object? isGreaterThanOrEqualTo,
    Object? arrayContains,
    Iterable<Object?>? arrayContainsAny,
    Iterable<Object?>? whereIn,
    Iterable<Object?>? whereNotIn,
    bool? isNull,
  }) async {
    if (_isListen) return;
    _isListen = true;
    (field != null
            ? collection.where(
                field,
                isEqualTo: isEqualTo,
                isNotEqualTo: isNotEqualTo,
                isLessThan: isLessThan,
                isLessThanOrEqualTo: isLessThanOrEqualTo,
                isGreaterThan: isGreaterThan,
                isGreaterThanOrEqualTo: isGreaterThanOrEqualTo,
                arrayContains: arrayContains,
                arrayContainsAny: arrayContainsAny,
                whereIn: whereIn,
                whereNotIn: whereNotIn,
                isNull: isNull,
              )
            : collection)
        .snapshots()
        .listen((e) {
      final patios =
          e.docs.map((e) => PatioModel.fromMap(e.data())).toList();
      patios.sort((a, b) => a.createdAt.compareTo(b.createdAt));
      dataStream.add(patios);
    });
  }

  PatioModel getById(String id) =>
      data.firstWhere((e) => e.id == id, orElse: () => PatioModel.empty());

  Future<PatioModel?> add(PatioModel model) async {
    await collection.doc(model.id).set(model.toMap());
    return model;
  }

  Future<PatioModel?> update(PatioModel model) async {
    await collection.doc(model.id).update(model.toMap());
    return model;
  }

  Future<void> delete(PatioModel model) async {
    await collection.doc(model.id).delete();
  }
}
